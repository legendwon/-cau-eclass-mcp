...
| 📅 **데일리 브리핑** | 오늘 할 일, 쪽지, 공지사항을 한 번에 요약 |
| 📝 **통합 상세 조회** | 과목별 강의(출석 포함), 과제, 자료실 정보 조회 |
| 🎥 **VOD 직링크** | 동영상 강의의 실제 스트리밍 주소 추출 |
| 📂 **지능형 다운로드** | 파일명 변경 및 과목별 폴더 자동 관리 다운로드 |
...
```
나: "e-class 오늘 무슨 일 있어?"
Claude: [오늘 마감인 과제, 읽지 않은 쪽지, 최신 공지사항 요약 표시]

나: "운체보 이번 주 강의 목록 보여줘"
Claude: [주차별 강의 목록 + 동영상 시청 완료 여부 표시]

나: "인공지능 과제 확인해줘"
Claude: [과제 목록 + 마감일 + 제출 상태 표시]

나: "1주차 강의안 다운로드해줘"
Claude: [파일명을 '1주차_강의안.pdf'로 바꾸고 '운체보' 폴더에 저장]

나: "강의 영상 링크 추출해줘"
Claude: [동영상 직링크와 화질별 URL 제공]
```
...
| `get_daily_briefing` | **(추천)** 통합 데일리 브리핑 | 없음 |
| `get_course_details` | 강의/과제/자료실 상세 조회 | `course_id`, `content_type` ('weekly', 'assignments', 'files') |
| `download_file` | 스마트 파일 다운로드 | `course_id`, `file_id`, `save_path` |
| `get_vod_info` | 동영상 스트리밍 링크 추출 | `content_id` |
...
중앙대는 Canvas 기반의 **CAU-ON**과 동영상/출석 관리를 위한 **LearningX**를 혼합하여 사용합니다. 본 MCP 서버는 이 두 시스템의 데이터를 통합하여 제공합니다.

1. CAU SSO 포탈에서 RSA 암호화로 로그인
2. API가 활성화된 세션 쿠키 및 JWT 토큰 획득
3. Canvas API 및 LearningX API (`xn_api_token`) 통합 요청
4. OCS XML 파싱을 통한 VOD 스트리밍 주소 추출
